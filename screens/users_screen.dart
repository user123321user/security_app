...
import '../main.dart'; // ربط كلاس السجلات المركزي الموحد
...
class _UsersScreenState extends State<UsersScreen> {
  ...
  // 1. عملية الإضافة: إنشاء حساب معزول سحابياً وحفظ الـ Role وتدوين النشاط
  ...
  // 2. عملية التعديل: دالة تحديث دور وصلاحية مستخدم وتدوين الحركة
  Future<void> _updateUserRole(String docId, String newRole, String userName) async {
    ...
    try {
      ...
      await AppLogger.logActivity(
        ...
        details: 'تم تعديل صلاحية الحساب ($userName) إلى [$newRole]',
        userEmail: currentAdminEmail,
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  // 3. عملية الحذف: دالة مسح مستند المستخدم من الـ Firestore وتدوينه
  Future<void> _deleteUser(String docId, String userName) async {
    ...
    try {
      ...
      await AppLogger.logActivity(
        ...
        details: 'تم مسح وإيقاف حساب المستخدم ($userName) بالكامل من النظام',
        userEmail: currentAdminEmail,
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  // نافذة منبثقة لتغيير الصلاحية ديناميكياً
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      String uName = data['name'] ?? 'بدون اسم';
