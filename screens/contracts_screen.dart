...
class _ContractsScreenState extends State<ContractsScreen> {
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          _selectedPaymentStatus = 'قيد الانتظار';
        });
  ...
  Stream<QuerySnapshot> _getContractsStream() {
    if (widget.userRole == 'admin') {
      ...
      return FirebaseFirestore.instance.collection('contracts').where('clientName', isEqualTo: widget.currentUserName).snapshots();
    }
  }
  @override
  Widget build(BuildContext context) {
    // [الحل الحاسم والقاطع]: تحديد مصدر الاتجاه من حزمة الـ material لكسر تضارب حزمة الـ PDF نهائياً
    return Directionality(
      textDirection: widget.isArabic ? material.TextDirection.rtl : material.TextDirection.ltr,
      child: Scaffold(
        appBar: AppBar(
              ...
              if (widget.userRole == 'admin') ...[
                StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    if (!snapshot.hasData) {
                      return DropdownButtonFormField(items: const [], onChanged: null, decoration: InputDecoration(labelText: AppStrings.get('loading_clients', widget.isArabic), border: const OutlineInputBorder()));
                    }
